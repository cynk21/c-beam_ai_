...
import { a as settings, c as config, W as WEBUI_NAME, u as user, r as models } from "../../../../../chunks/index2.js";
...
const Models = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${$$result.head += `<!-- HEAD_svelte-1nyg6gy_START -->${$$result.title = `<title> ${escape($i18n.t("Models"))} • ${escape($WEBUI_NAME)} </title>`, ""}<!-- HEAD_svelte-1nyg6gy_END -->`, ""} ${`<div class="w-full h-full flex justify-center items-center">${validate_component(Spinner, "Spinner").$$render($$result, {}, {}, {})}</div>`}`;
  } while (!$$settled);
  $$unsubscribe_i18n();
